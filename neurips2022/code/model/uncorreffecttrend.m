...
prior.cov  = {@priorDelta, ... % 1:  group trend length scale
              ...
              @priorDelta, ...                      % 4
              @priorDelta, ...               % 5: full effect time
              {@priorSmoothBox2, -4, -1, 5}, ...    % 6: effect output scale
              {@priorTransform,@exp,@exp,@log,{@priorGamma,10,2}}, ... % 7: x length scale
              {@priorSmoothBox2, -4, -1, 5}};       % 8: x output scale
prior.lik  = {{@priorSmoothBox2, -4, -1, 5}};       % 9: noise std
prior.mean = {@priorDelta, [], []};                 % 10: mean
...
filename = "./data/synthetic/uncorreffecttrend_" + HYP + "_SEED_" + SEED + ".pdf";
set(fig, 'PaperPosition', [0 0 10 10]); 
set(fig, 'PaperSize', [10 10]);
print(fig, filename, '-dpdf','-r300');
close;

save("./data/synthetic/uncorreffecttrend_" + HYP + "_SEED_" + SEED + ".mat");
