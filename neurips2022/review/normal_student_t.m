...
theta.lik = [log(4-1), log(noise_scale)];
prior.lik  = { {@priorDelta}, ... 
    {@priorSmoothBox2, -4, -1, 5}};       % 15: noise std
inference_method = {@infPrior, @infLaplace, prior};
theta_t = minimize_v2(theta, @gp, p, inference_method, mean_function, ...
                    covariance_function, @likT, x, y);

ll_t = gp(theta_t, inference_method, mean_function, ...
                    covariance_function, @likT, x, y);

BIC_gauss = 13*log(size(x,1)) + 2*ll_gauss;
BIC_t = 14*log(size(x,1)) + 2*ll_t;