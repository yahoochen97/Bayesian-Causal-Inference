...
function [nlZ, dnlZ] = gp_likelihood_independent(hyperparameters, ...
  ...
  counter = size(priors, 1);
  nfirm = size(hyperparameters.mean, 1) - 2*counter;
  ...
  for i = 1:num_samples
    ...
    hyp = full2one(hyperparameters, id, counter, nfirm);
    
    if (nargout == 1)
      this_nlZ = ...
          gp(hyp, im, mean_function, ...
             covariance_function, likelihood, xs{i}, ys{i});
    else
      [this_nlZ, this_dnlZ] = ...
          gp(hyp, im, mean_function, ...
             covariance_function, likelihood, xs{i}, ys{i});
        this_dnlZ = dnlZ2full(this_dnlZ, id, counter, nfirm);
    end
